...
-- 		["Ped"] = {
-- 			["Model"] = "u_m_y_juggernaut_01",
-- 			["Coords"] = vec3(-1041.01,-481.96,36.7),
-- 			["Heading"] = 354.34
-- 		},
-- 		["Respawn"] = {
-- 			{ -1143.61,-3392.98,13.93,150.24 }
-- 		},
-- 		["Coords"] = {
-- 			{ -1153.21,-3412.61,15.24 },
-- 			{ -1150.27,-3414.31,16.45 },
-- 			{ -1148.19,-3415.51,18.88 },
-- 			{ -1150.57,-3414.13,18.95 },
-- 			{ -1153.27,-3412.57,19.95 },
-- 			{ -1156.12,-3410.93,19.4 },
-- 			{ -1158.47,-3409.57,18.91 },
-- 			{ -1156.95,-3410.44,17.6 },
-- 			{ -1158.43,-3409.59,16.87 },
-- 			{ -1158.26,-3409.69,14.68 },
-- 			{ -1154.79,-3411.69,14.54 },
-- 			{ -1149.97,-3414.48,14.66 },
-- 			{ -1147.6,-3415.84,14.73 },
-- 			{ -1148.0,-3415.61,15.95 }
-- 		},
-- 		["Ball"] = nil,
-- 		["Mode"] = "Speedball",
-- 		["Exit"] = { -1040.76,-480.58,36.72 },
-- 		["Route"] = 900002,
-- 		["PolyZone"] = PolyZone:Create({
-- 			vector2(-1136.01,-3395.43),
-- 			vector2(-1146.58,-3415.45),
-- 			vector2(-1159.04,-3408.22),
-- 			vector2(-1147.80,-3388.69)
-- 		},{ name = "Speedball" })
-- 	}
-- }
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("arena:Enter",function(Number)
    if Arena == "0" and vSERVER.CheckEnter(Zones[Number]["Route"],Number) then
        ...
        if Zones[Arena]["Mode"] == "Deathmath" then
            LocalPlayer["state"]["Route"] = Zones[Arena]["Route"]
            SendNUIMessage({ Action = "Show", Mode = Zones[Arena]["Mode"] })
            TriggerServerEvent("arena:Players","+",Zones[Arena]["Route"])
        elseif Zones[Arena]["Mode"] == "Speedball" then
            SendNUIMessage({ Action = "Show", Mode = Zones[Arena]["Mode"] })
...
AddEventHandler("arena:Exit",function()
    if Arena ~= "0" then
        ...
        if Zones[Arena]["Mode"] == "Speedball" and DoesEntityExist(Zones[Arena]["Ball"]) then
            DeleteEntity(Zones[Arena]["Ball"])
        end
        
        SetEntityCoords(Ped,Zones[Arena]["Exit"][1],Zones[Arena]["Exit"][2],Zones[Arena]["Exit"][3],false,false,false,false)
        SendNUIMessage({ Action = "Hide", Mode = Zones[Arena]["Mode"] })
...
AddEventHandler("arena:Players",function(Route,Amount)
    if Arena ~= "0" and LocalPlayer["state"]["Route"] == Route then
        Players = Amount
        SendNUIMessage({ Action = "Score", Mode = Zones[Arena]["Mode"], Players = Players, Streek = KillStreek })
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ARENA:RESETSTREEK
...
AddEventHandler("arena:ResetStreek",function()
    if Arena ~= "0" then
        SendNUIMessage({ Action = "Score", Mode = Zones[Arena]["Mode"], Players = Players, Streek = 0 })
        KillStreek = 0
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("gameEventTriggered",function(Name,Message)
    if Name == "CEventNetworkEntityDamage" and Arena ~= "0" and Zones[Arena]["Mode"] == "Deathmath" then
        if GetEntityHealth(Message[1]) <= 100 and PlayerPedId() == Message[2] and IsPedAPlayer(Message[1]) then
            SendNUIMessage({ Action = "Score", Mode = Zones[Arena]["Mode"], Players = Players, Streek = KillStreek + 1 })
...
AddEventHandler("entityDamaged",function(Entity,Player)
    if Arena ~= "0" and Zones[Arena]["Mode"] == "Speedball" and Player == PlayerPedId() and Entity == Zones[Arena]["Ball"] then
        SendNUIMessage({ Action = "Score", Mode = Zones[Arena]["Mode"] })
        CreateBall("v_ilev_exball_grey")
    end
end)
