...
AddEventHandler("police:Impound",function(entity)
	...
	if Passport and not Active[Passport] then
		...
		if not Impound[entity[2].."-"..entity[1]] then
			...
			-- TriggerClientEvent("Notify2",source,"#veiculoRegistrado")
			...
			-- TriggerClientEvent("Notify2",source,"#veiculoJaLista")
		end
...
function runPlate(source,Plate)
	...
	if Passport then
		...
		-- TriggerClientEvent("Notify2",source,"#passaporteIdentity",{msg=Identity["id"],msg2=Identity["name"],msg3=Identity["name2"],msg4=Identity["phone"]})
	else
		...
		-- TriggerClientEvent("Notify2",source,"#passaporteNome",{msg=plateSave[Plate][1],msg2=plateSave[Plate][2]})
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- POLICE:ARREST
			...
			if Passport then
				...
				if Vehicle[1] then
					if Vehicle[1]["arrest"] <= os.time() then
						...
						-- TriggerClientEvent("Notify2",source,"#veicArrest")
					else
						TriggerClientEvent("Notify",source,"amarelo","Veículo já se encontra apreendido.",5000)
						-- TriggerClientEvent("Notify2",source,"#veicArrested")
					end
				end
			end
		end
