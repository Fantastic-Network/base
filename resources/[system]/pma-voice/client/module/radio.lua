...
function setRadioChannel(channel)
	...
	TriggerServerEvent('pma-voice:setPlayerRadio2', channel)
...
RegisterCommand('+radiotalk', function()
    ...
    if LocalPlayer["state"]["temporaryBanned"] then return end
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) then
                ...
                -- TriggerServerEvent("addEntity","prop_cs_hand_radio")
