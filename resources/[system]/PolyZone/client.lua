...
function PolyZone:draw()
  ...
  local aOpacity = self.debugColors.opacity or 48
  local noLines = self.debugColors.noLines
  ...
  for i=1, #points do
    ...
    if i < #points then
      ...
      _drawWall(point, p2, minZ, maxZ, wR, wG, wB, aOpacity)
    end
  end
  ...
  if #points > 2 then
    ...
    _drawWall(firstPoint, lastPoint, minZ, maxZ, wR, wG, wB, aOpacity)
  end
end
...
local function _drawGrid(poly)
  ...
  local noLines = poly.debugColors.noLines
